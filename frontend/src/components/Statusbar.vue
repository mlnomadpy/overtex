...
<template>
  <div class="border-t bg-background px-4 py-1 flex items-center justify-between text-xs">
    <!-- Left: Status -->
    <div class="flex items-center gap-4">
      <div class="flex items-center gap-2" :class="statusClass">
        <component :is="statusIcon" class="h-3 w-3" :class="{ 'animate-spin': isBuilding }" />
        <span class="font-medium">{{ statusText }}</span>
      </div>
      
      <div v-if="buildError" class="text-red-500 max-w-md truncate">
        {{ buildError }}
      </div>
    </div>
